...
export default function AsEditPage() {
  ...
  useEffect(() => {
    const loadReceipt = async () => {
      ...
      if (!response.ok) {
        setMessage(result.error || result.message || 'A/S ?•ë³´ë¥?ë¶ˆëŸ¬?¤ì? ëª»í–ˆ?µë‹ˆ??');
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        setSubmitStage('error');
        setMessage(result.error || result.message || 'A/S ?˜ì •???¤íŒ¨?ˆìŠµ?ˆë‹¤.');
        setErrorDetails({ requestId: result.requestId, status: response.status, body: result });
      ...
      setSubmitStage('done');
      setMessage(result.message || 'A/S ?˜ì •???„ë£Œ?˜ì—ˆ?µë‹ˆ??');
      setReceipt(result.data);
      addLog('success', 'A/S ?˜ì • ?„ë£Œ');
    ...
    return (
      <main className="space-y-6">
        ...
        <section className="rounded-xl bg-white p-6 shadow-sm">
          <p className="text-sm text-slate-500">{message ?? 'A/S ?•ë³´ë¥?ë¶ˆëŸ¬?¤ëŠ” ì¤?..'}</p>
          {errorDetails && (
  ...
  return (
    <>
      <main className="space-y-6">
        ...
        <section className="rounded-xl bg-white p-6 shadow-sm">
          <div className="mb-6">
            <h2 className="text-lg font-semibold">A/S ?˜ì •</h2>
      ...
      {showDebugPanel && <DebugPanel logs={logs} title="A/S ?˜ì • ë¡œê·¸" onClear={() => setLogs([])} />}
    </>
  );
}
